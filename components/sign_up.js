...
export default function SignUp({ error, loading, password, setPassword, setUsername, submit, username }) {
  if (error) {
    error = (
  ...
  return (
    <form className="max-w-sm mx-auto" onSubmit={submit}>
      ...
      <p className="mt-4 text-sm text-center">
        {"Already have an account?"} <Link href="/"><a className="font-normal">Sign in.</a></Link>
      </p>
    </form>
  );
}
